...
- [ ] IMPORTANT!!! Revert previous change of adding `hostname` to the `endpoints` table. After further consideration, it may be better to 2`keep the `endpoints` object storing only `address` and specific checks will resolve IP if needed. For example:
